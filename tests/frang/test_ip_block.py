...
from impacket.examples.ntlmrelayx.servers.socksserver import activeConnectionsWatcher
...
""",
    ...
    async def test_on(self):
        ...
        await self.assertFrangWarning(warning="Warning: block client:", expected=1)
        await self.assertFrangWarning(
            warning="frang: Host header field contains IP address", expected=1
        )

    async def test_blocktime_expired(self):
        self.disable_deproxy_auto_parser()
        await self.set_frang_config(frang_config="http_strict_host_checking true;\nip_block 11;")
        c1 = self.get_client("same-ip1")
        c2 = self.get_client("same-ip2")
    ...
    async def test_off(self):
        ...
        await self.assertFrangWarning(warning="Warning: block client:", expected=0)
        await self.assertFrangWarning(
            warning="frang: Host header field contains IP address", expected=1
        )
...
""",
    ...
    async def test_on(self):
        ...
        await c3.send_request(self.REQ)
        ...
        await self.assertFrangWarning(warning="Warning: block client:", expected=1)
        await self.assertFrangWarning(warning=self.warning_msg, expected=1)
    ...
    async def test_off(self):
        ...
        # Blocking is off: clients with the same IPs
        # handled separately
        await c1.send_request(self.REQ, "200")
