...
class FrangHttpHeaderCountTestCase(FrangTestCase):
    ...
    async def test_reaching_the_limit(self):
        ...
        await self.check_response(client, status_code="403", warning_msg=ERROR)
    ...
    async def test_not_reaching_the_limit(self):
        ...
        await self.check_response(client, status_code="200", warning_msg=ERROR)

    async def test_not_reaching_the_limit_2(self):
        """
        We set up for Tempesta `http_header_cnt 6` and
        made request with 4 headers
        """
        client = await self.base_scenario(frang_config="http_header_cnt 6;", requests=self.requests)
        await self.check_response(client, status_code="200", warning_msg=ERROR)

    async def test_default_http_header_cnt(self):
        """
        We set up for Tempesta default `http_header_cnt` and
        made request with many headers
        """
        client = await self.base_scenario(frang_config="", requests=self.request_with_many_headers)
        await self.check_response(client, status_code="200", warning_msg=ERROR)
...
class FrangHttpHeaderCountH2(H2Config, FrangHttpHeaderCountTestCase):
    ...
    async def test_reaching_limit_headers_as_bytes(self, name, huffman):
        """
        ...
        await self.check_last_response(client, status_code="403", warning_msg=ERROR)

    @marks.Parameterize.expand(
        [marks.Param(name="huffman", huffman=True), marks.Param(name="no_huffman", huffman=False)]
    )
    async def test_not_reaching_limit_headers_as_bytes(self, name, huffman):
        """
    ...
    async def test_not_reaching_the_limit_2(self, name, huffman):
        """
        We set up for Tempesta `http_header_cnt 8` and
        made request with 8 headers (duplicate headers taken into account)
        as index from dynamic table.
    ...
    async def test_default_http_header_cnt(self, name, huffman):
        """
        We set up for Tempesta default `http_header_cnt` and
    ...
    async def test_hpack_bomb(self, name, huffman):
        ...
        """
        await self.set_frang_config("http_header_cnt 6;")
