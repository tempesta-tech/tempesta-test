...
class FrangHostRequiredTestCase(FrangTestCase):
    ...
    async def test_host_header_set_ok(self):
    ...
    async def test_host_header_with_old_proto(self):
        """
    ...
    async def test_host_header_mismatch(self):
        """Test with mismatched header `host`."""
        client = await self.base_scenario(
            frang_config="http_strict_host_checking true;",
            requests=["GET http://tempesta-tech.com/ HTTP/1.1\r\nHost: example.com\r\n\r\n"],
        )
        await self.check_response(client, status_code="403", warning_msg=WARN_DIFFER)

    async def test_host_header_mismatch_empty(self):
        """
    ...
    async def test_host_header_no_port_in_uri(self):
        ...
        await self.check_response(client, status_code="200", warning_msg=WARN_DIFFER)

    async def test_host_header_no_port_in_host(self):
    ...
    async def test_host_header_mismatch_port_in_host(self):
        ...
        await self.check_response(client, status_code="403", warning_msg=WARN_PORT_DIFF)

    async def test_auto_port_mismatch(self):
        """
    ...
    async def test_auto_port_mismatch_uri(self):
        """
    ...
    async def test_host_header_as_ip(self):
        """Test with header `host` as ip address."""
        client = await self.base_scenario(
            frang_config="http_strict_host_checking true;",
            requests=["GET / HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n"],
        )
        await self.check_response(client, status_code="403", warning_msg=WARN_IP_ADDR)

    async def test_host_header_as_ip6(self):
    ...
    async def test_disabled_host_http_required(self):
    ...
    async def test_default_host_http_required(self):
...
""",
    ...
    async def test_h2_header_ok(self):
        """Test with header `host`, success."""
        await self.set_frang_config(frang_config="http_strict_host_checking true;")
    ...
    async def test_h2_host_header_as_ip(self):
        """Test with header `host` as ip address."""
        await self._test(
            headers=[
                (":path", "/"),
    ...
    async def test_h2_authority_header_as_ip(self):
        """Test with header `host` as ip address."""
        await self._test(
            headers=[
                (":path", "/"),
    ...
    async def test_h2_host_header_as_ipv6(self):
        """Test with header `host` as ip v6 address."""
        await self._test(
            headers=[
                (":path", "/"),
    ...
    async def test_h2_authority_header_as_ipv6(self):
        """Test with header `host` as ip v6 address."""
        await self._test(
            headers=[
                (":path", "/"),
    ...
    async def test_h2_different_host_and_authority_header(self):
        await self._test(
    ...
    async def _test(
        ...
        await self.check_response(client, status_code=status_code, warning_msg=expected_warning)
