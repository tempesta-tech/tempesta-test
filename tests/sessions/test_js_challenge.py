...
class BaseJSChallenge(tester.TempestaTest):
    ...
    @staticmethod
    async def _java_script_sleep(cookie: str) -> None:
        """This repeats sleep from JavaScript in response body"""
        await asyncio.sleep(BaseJSChallenge._java_script_sleep_time(cookie))
    ...
    async def process_first_js_challenge_req(self, client):
        """
        ...
        """
        await client.send_request(self.prepare_first_req(client), "503")
...
class JSChallenge(BaseJSChallenge):
    ...
    async def _test_first_request(
        self, client, request, method, accept, status, conn_is_closed=False
    ):
    ...
    async def test_first_request(self, name, method, accept, status):
        """
        ...
        await self._test_first_request(client, request, method, accept, status)
    ...
    async def test_servicing_non_challengeble_request_from_cache(self, name, method, status):
        """
        ...
        """
        await self.test_second_request_GET_and_accept_all()
    ...
    async def test_second_request(self, name, method, accept):
        """Tempesta should not check a request if max_misses > 0."""
        await self.start_all_services()
        client = self.get_client("client-1")
        cookie = await self.process_first_js_challenge_req(client)
    ...
    async def test_delay(self, name, sleep, second_status):
        """
    ...
    async def test_restart_js_challenge(self):
        """
        ...
        await self._java_script_sleep(cookie_1[1])
    ...
    async def test_number_of_requests_is_greater_than_max_misses(self):
        """
    ...
    async def test_enable_challenge_on_reload(self):
        """
        ...
        await client.send_request(self.prepare_second_req(client, cookie), "200")

    async def test_not_block_after_tempesta_restart(self):
        """This case repeats the blocking of the browser on the next day."""
        await self.start_all_services()
        client = self.get_client("client-1")

        # browser make request and wait for time from JS challenge
        cookie_1 = await self.process_first_js_challenge_req(client)
        await self._java_script_sleep(cookie_1[1])
        ...
        await self._java_script_sleep(cookie_2[1])
        await client.send_request(
            self.prepare_second_req(client, cookie_2), expected_status_code="200"
        )

    async def test_new_client_session_with_exist_cookie(self):
    ...
    async def test_delay_pipelined(self, name, sleep, conn_is_closed):
        await self.start_all_services()
        ...
        responses = await self.client_send_pipelined_requests(client, [request, request], False)
    ...
    async def test_first_post_request_pipelined(self):
        ...
        responses = await self.client_send_pipelined_requests(client, [request1, request2], False)
    ...
    async def test_cookies_in_request(self, name, single):
        ...
        cookie1 = await self.process_first_js_challenge_req(client)
        await self._java_script_sleep(cookie1[1])
        cookie2 = await self.process_first_js_challenge_req(client)
        await self._java_script_sleep(cookie2[1])
        ...
        await client.send_request(request, "500")
    ...
    async def test_resp_code(self, name, resp_code, expected_status):
        ...
        client = self.get_client("client-1")
        await client.send_request(self.prepare_first_req(client), expected_status)
    ...
    async def test_method_override(self, name, method, override, status, conn_is_closed):
        await self.start_all_services()
        ...
        await self._test_first_request(client, request, method, "text/html", status, conn_is_closed)
    ...
    async def test_method_override_with_cache(self, name, method, override, status):
        await self.test_second_request_GET_and_accept_all()
...
class JSChallengeCookieExpiresAndMethodOverride(BaseJSChallenge):
    ...
    async def test_cookie_expires(self):
        await self.start_all_services()
        ...
        self.assertTrue(
            await self.klog.find(
                "http_sess: sticky cookie value expired", cond=dmesg.amount_equals(1)
            ),
            1,
        )
