...
class MessageTransformations(tester.TempestaTest):
    ...
    def test_small_body_to_chunked(self):
        ...
        self.assertFalse(client.connection_is_closed(), "Client connection was unexpectedly closed")
        self.assertEqual(rec_resp.status, "200", "Unexpected response")
        self.assertEqual(
            rec_resp.headers.get("Transfer-Encoding", None),
            "chunked",
            "Chunked encoding is missed!",
        )
        self.assertEqual(rec_resp.body, self.chunked_body(body), "Body is not in chunked encoding!")
    ...
    def test_transform_not_possible(self):
        ...
        self.assertTrue(
            ...
            "Client connection was not closed to indicate message" " end",
        )
        self.assertEqual(rec_resp.status, "200", "Unexpected response")
        self.assertEqual(
            rec_resp.headers.get("Transfer-Encoding", None),
            "chunked, gzip",
            "Unexpected transfer encoding!",
        )
        self.assertEqual(rec_resp.body, body, "Body is incorrectly modified!")
