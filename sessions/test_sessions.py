...
class StickySessionsFailover(StickySessions):
    ...
    def test_sessions(self):
        ...
        self.assertIn(failovered_s_id, ['server-1', 'server-2'],
                      "session is pinned to unexpected server")
    ...
    def test_sessions_reserved_server(self):
        ...
        failovered_s_id = self.client_send_next_req(client, cookie)
        self.assertIn(failovered_s_id, ['server-3', 'server-4'],
                      "session is pinned to unexpected server")

        srv1.start()
        srv2.start()
        self.assertTrue(srv1.wait_for_connections(timeout=1),
                        "Can't restart backend server")
        self.assertTrue(srv2.wait_for_connections(timeout=1),
                        "Can't restart backend server")
        for _ in range(ATTEMPTS):
            new_s_id = self.client_send_next_req(client, cookie)
            self.assertEqual(failovered_s_id, new_s_id,
