...
class TLSClientAutomata(Automaton):
    ...
    def do_send_client_hello(self):
        client_hello = TLSRecord(version=self.tls_version) / TLSHandshakes(
            handshakes=[
                TLSHandshake()
                / TLSClientHello(
    ...
    def recv_server_hello(self):
        ...
        self.debug(
            10,
            "CIPHER: %s"
            % TLS_CIPHER_SUITES.get(p[TLSServerHello].cipher_suite, p[TLSServerHello].cipher_suite),
        )
    ...
    def do_send_client_key_exchange(self):
        ...
        client_key_exchange = TLSRecord(version=tls_version) / TLSHandshakes(
            handshakes=[TLSHandshake() / self.tlssock.tls_ctx.get_client_kex_data()]
        )
    ...
    def send_client_finish(self):
        self.tlssock.sendall(
            TLSHandshakes(
                handshakes=[
                    TLSHandshake() / TLSFinished(data=self.tlssock.tls_ctx.get_verify_data())
                ]
            )
        )
    ...
    def recv_server_finish(self):
        p = self.tlssock.recvall(timeout=self.timeout)
        if not (
            p.haslayer(TLSFinished)
            or (
                p.haslayer(TLSPlaintext)
                and SSL(str(TLSRecord(content_type="handshake") / p[TLSPlaintext].data)).haslayer(
                    TLSFinished
                )
            )
        ):
    ...
    def recv_server_appdata(self):
        p = self.tlssock.recvall(timeout=self.timeout)
        if not (
            p.haslayer(TLSRecord) and p[TLSRecord].content_type == TLSContentType.APPLICATION_DATA
        ):
            raise self.ERROR(p)
...
class TLSServerAutomata(Automaton):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Automaton.__init__(self, *args, **kwargs)
        self.STATES = {
            TLSClientHello: "CLIENT_HELLO_RECV",
        ...
        self.ACTIONS = {
            TLSClientHello: "recv_client_hello",
    ...
    def do_bind(self):
        ...
        for key_pk in (k for k in pemo.keys() if "PRIVATE" in k.upper()):
            self.srv_tls_sock.tls_ctx.server_ctx.asym_keystore = tlsk.RSAKeystore.from_private(
                pemo[key_pk].get("full")
            )
            break
    ...
    def do_send_server_hello(self):
        server_hello = TLSRecord(version=self.tls_version) / TLSHandshakes(
            handshakes=[
                TLSHandshake()
                / TLSServerHello(
                    version=self.tls_version,
    ...
    def do_send_server_certificates(self):
        ...
        server_certificates = TLSRecord(version=self.tls_version) / TLSHandshakes(
            handshakes=[
                TLSHandshake()
                / TLSCertificateList()
                / cls_cert(certificates=[TLSCertificate(data=x509.X509_Cert(self.dercert))])
            ]
        )
    ...
    def do_send_server_hello_done(self):
        server_hello_done = TLSRecord(version=self.tls_version) / TLSHandshakes(
            handshakes=[TLSHandshake() / TLSServerHelloDone()]
        )
    ...
    def END(self):
        self.tlssock.sendall(
            TLSAlert(level=TLSAlertLevel.WARNING, description=TLSAlertDescription.CLOSE_NOTIFY)
        )
