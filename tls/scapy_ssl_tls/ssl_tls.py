...
class BLenField(LenField):
    __slots__ = ["adjust_i2m", "adjust_m2i", "numbytes", "length_of", "count_of"]
    ...
    def __init__(
        ...
        default,
        fmt="I",
        adjust_i2m=lambda pkt, x: x,
        numbytes=None,
        length_of=None,
        count_of=None,
        adjust_m2i=lambda pkt, x: x,
    ):
...
TLS_HANDSHAKE_TYPES.update(
    {0x6: "hello_retry_request", 0x8: "encrypted_extensions", 0x18: "key_update"}
)
TLSHandshakeType = EnumStruct(TLS_HANDSHAKE_TYPES)

TLS_EXTENSION_TYPES = registry.EXTENSIONTYPE_VALUES
TLS_EXTENSION_TYPES.update(
    {
...
)  # manually add NPN as it is not in iana registry
...
TLS_ALERT_DESCRIPTIONS.update(
    {1: "end_of_early_data", 109: "missing_extension", 116: "certificate_required"}
)
...
TLS_PSK_KEY_EXCHANGE_MODE.update({0: "psk_ke", 1: "psk_dhe_ke", 255: "reserved"})
...
class TLSRecord(StackedLenPacket):
    ...
    name = "TLS Record"
    fields_desc = [
        ByteEnumField("content_type", TLSContentType.APPLICATION_DATA, TLS_CONTENT_TYPES),
...
class TLSServerName(PacketNoPayload):
    name = "TLS Servername"
    fields_desc = [
        ByteEnumField("type", 0x00, {0x00: "host"}),
...
class TLSExtALPN(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="protocol_name_list", fmt="H"),
        PacketListField(
            "protocol_name_list", DEFAULT_ALPN_LIST, TLSALPNProtocol, length_from=lambda x: x.length
        ),
    ]
...
class TLSExtension(PacketLengthFieldPayload):
    ...
    name = "TLS Extension"
    fields_desc = [
        XShortEnumField("type", TLSExtensionType.SERVER_NAME, TLS_EXTENSION_TYPES),
        XLenField("length", None, fmt="!H"),
    ]
...
class TLSURLAndOptionalHash(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("url_length", None, length_of="url", fmt="H"),
...
class TLSExtCertificateURL(PacketNoPayload):
    ...
    fields_desc = [
        ByteEnumField("type", TLSCertChainType.INDIVIDUAL_CERTS, TLS_CERT_CHAIN_TYPE),
        XFieldLenField("length", None, length_of="certificate_urls", fmt="H"),
        PacketListField(
            "certificate_urls", None, TLSURLAndOptionalHash, length_from=lambda x: x.length
        ),
    ]
...
class TLSExtSupportedGroups(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="named_group_list", fmt="H"),
        ReprFieldListField(
            "named_group_list",
            DEFAULT_NAMED_GROUP_LIST,
...
class TLSExtSignatureAlgorithms(PacketNoPayload):
    ...
    fields_desc = [
        ...
        ReprFieldListField(
            "algs",
            DEFAULT_SIG_SCHEME_LIST,
            ShortEnumField("length", None, TLS_SIGNATURE_SCHEMES),
            length_from=lambda x: x.length,
        ),
    ]
...
class TLSExtHeartbeat(PacketNoPayload):
    ...
    fields_desc = [
        ByteEnumField("mode", TLSHeartbeatMode.PEER_NOT_ALLOWED_TO_SEND, TLS_HEARTBEAT_MODE)
    ]
...
class TLSOCSPResponderID(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="responder_id", fmt="H"),
...
class TLSExtCertificateStatusRequest(PacketNoPayload):
    ...
    fields_desc = [
        ByteEnumField("status_type", TLSCertificateStatusType.OCSP, TLS_CERTIFICATE_STATUS_TYPES),
        XFieldLenField("responder_id_length", None, length_of="responder_id_list", fmt="H"),
        PacketListField(
            "responder_id_list",
            None,
            TLSOCSPResponderID,
            length_from=lambda x: x.responder_id_length,
        ),
...
class TLSExtSupportedVersions(PacketNoPayload):
    ...
    fields_desc = [
        ...
        ReprFieldListField(
            "versions",
            [TLSVersion.TLS_1_3],
            XShortEnumField("version", None, TLS_VERSIONS),
            length_from=lambda x: x.length,
        ),
    ]
...
class TLSExtCookie(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="cookie", fmt="H"),
...
class TLSClientHelloKeyShare(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="client_shares", fmt="H"),
...
class TLSExtKeyShare(Packet):
    ...
    fields_desc = []
    type_map = {
        "TLSHelloRetryRequest": TLSHelloRetryRequestKeyShare,
...
class TLSPSKIdentity(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="identity", fmt="H"),
...
class TLSPSKBinderEntry(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="binder_entry", fmt="B"),
...
class TLSClientHelloPreSharedKey(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("identities_length", None, length_of="identities", fmt="H"),
        PacketListField(
            "identities", None, TLSPSKIdentity, length_from=lambda x: x.identities_length
        ),
...
class TLSExtPreSharedKey(Packet):
    ...
    fields_desc = []
    type_map = {
        "TLSClientHello": TLSClientHelloPreSharedKey,
...
class TLSExtPSKKeyExchangeModes(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="ke_modes", fmt="B"),
        ReprFieldListField(
            "ke_modes",
            [TLSPSKKeyExchangeMode.PSK_DHE_KE],
            ByteEnumField("ke_mode", None, TLS_PSK_KEY_EXCHANGE_MODE),
            length_from=lambda x: x.length,
        ),
    ]
...
class TLSHelloRetryRequest(Packet):
    ...
    fields_desc = [
        XShortEnumField("version", TLSVersion.TLS_1_3, TLS_VERSIONS),
...
class TLSClientHello(PacketNoPayload):
    ...
    fields_desc = [
        XShortEnumField("version", TLSVersion.TLS_1_2, TLS_VERSIONS),
        ...
        XFieldLenField(
            "compression_methods_length", None, length_of="compression_methods", fmt="B"
        ),
        ReprFieldListField(
            "compression_methods",
        ...
        StrConditionalField(
            ...
            if val or pkt.extensions or (s and struct.unpack("!H", s[:2])[0] == len(s) - 2)
            else False,
        ),
        TypedPacketListField(
            "extensions",
...
class TLSServerHello(PacketNoPayload):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrFixedLenField("random", os.urandom(32), 32),
            lambda pkt: pkt.version >= TLSVersion.TLS_1_3,
        ),
...
class TLSHeartBeat(PacketNoPayload):
    ...
    fields_desc = [
        ByteEnumField(
            "type", TLSHeartbeatMessageType.HEARTBEAT_REQUEST, TLS_HEARTBEAT_MESSAGE_TYPE
        ),
...
class TLSClientKeyExchange(TLSKeyExchange):
    ...
    kex_payload_table = {
        TLSKexNames.RSA: TLSClientRSAParams,
...
class TLSServerDHParams(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("p_length", None, length_of="p", fmt="!H"),
...
class TLSServerECDHParams(PacketNoPayload):
    ...
    fields_desc = [
        ByteEnumField("curve_type", TLSECCurveTypes.NAMED_CURVE, TLS_EC_CURVE_TYPES),
...
class TLSServerKeyExchange(TLSKeyExchange):
    ...
    kex_payload_table = {TLSKexNames.DHE: TLSServerDHParams, TLSKexNames.ECDHE: TLSServerECDHParams}
    ...
    def guess_payload_class(self, payload):
        ...
        if (
            dh_params.p_length == len(dh_params.p)
            and dh_params.g_length == len(dh_params.g)
            and dh_params.ys_length == len(dh_params.y_s)
            and dh_params.sig_length == len(dh_params.sig)
        ):
            return TLSServerDHParams
        elif ecdh_params.p_length == len(ecdh_params.p) and ecdh_params.sig_length == len(
            ecdh_params.sig
        ):
...
class TLSCertificate(PacketNoPayload):
    ...
    fields_desc = [
        XBLenField("length", None, length_of="data", fmt="!I", numbytes=3),
...
class TLSCertificateEntry(PacketNoPayload):
    ...
    fields_desc = [
        XBLenField("length", None, length_of="cert_data", fmt="!I", numbytes=3),
        PacketLenField("cert_data", None, x509.X509_Cert, length_from=lambda x: x.length),
        XFieldLenField("extensions_length", None, length_of="extensions", fmt="H"),
        PacketListField(
            "extensions", None, TLSExtension, length_from=lambda x: x.extensions_length
        ),
    ]
...
class TLS13Certificate(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("request_context_length", None, length_of="request_context", fmt="B"),
...
class TLSCertificateList(Packet):
    ...
    def guess_payload_class(self, payload):
        ...
        if tls13_cert.request_context_length == len(
            tls13_cert.request_context
        ) and tls13_cert.length == certs_len(tls13_cert):
...
class TLSCertificateVerify(PacketNoPayload):
    ...
    fields_desc = [
        ShortEnumField("alg", TLSSignatureScheme.RSA_PKCS1_SHA256, TLS_SIGNATURE_SCHEMES),
...
class TLSCertificateType(PacketNoPayload):
    ...
    fields_desc = [
        ByteEnumField(
            "type", TLSCertificateTypeIdentifier.RSA_SIGN, TLS_CERTIFICATE_TYPE_IDENTIFIERS
        )
    ]
...
class TLSCADistinguishedName(PacketNoPayload):
    ...
    fields_desc = [
        XFieldLenField("length", None, length_of="dn", fmt="H"),
        PacketLenField("ca_dn", None, x509.X509_ExtDefault, length_from=lambda x: x.length),
    ]
...
class TLSCertificateRequest(Packet):
    ...
    fields_desc = [
        XFieldLenField("type_length", None, length_of="types", fmt="B"),
        PacketListField(
            "types", TLSCertificateType(), TLSCertificateType, length_from=lambda x: x.type_length
        ),
        XFieldLenField("alg_length", None, length_of="algs", fmt="H"),
        ReprFieldListField(
            "algs",
            DEFAULT_SIG_SCHEME_LIST,
            ShortEnumField("alg", None, TLS_SIGNATURE_SCHEMES),
    ...
    def pre_dissect(self, raw_bytes):
        ...
        if self.tls_ctx is not None:
            ...
            if self.tls_ctx.sec_params.cipher_mode_name == tlsc.CipherMode.CBC:
                ...
                data = raw_bytes[
                    self.tls_ctx.sec_params.GCM_EXPLICIT_IV_SIZE : -self.tls_ctx.sec_params.GCM_TAG_SIZE
                ]
        ...
        # Try and obtain the context from the underlying Record context
            ...
            if (
                self.tls_ctx is None
                and self.underlayer is not None
                and isinstance(self.underlayer, TLSRecord)
            ):
...
class TLSHandshake(PacketLengthFieldPayload):
    ...
    name = "TLS Handshake"
    fields_desc = [
        ByteEnumField("type", TLSHandshakeType.CLIENT_HELLO, TLS_HANDSHAKE_TYPES),
...
class SSLv2Record(Packet):
    ...
    fields_desc = [
        XBLenField(
            ...
            None,
            fmt="!H",
            adjust_i2m=lambda pkt, x: x | 0x8000,
            adjust_m2i=lambda pkt, x: x & 0x7FFF,
        ),  # hint SSLv2Record with MSB=1, all other bits=length
...
class SSLv2ClientMasterKey(Packet):
    ...
    fields_desc = [
        XBEnumField(
            "cipher_suite",
            SSLv2CipherSuite.RC4_128_WITH_MD5,
            SSLv2_CIPHER_SUITES,
            fmt="!I",
            numbytes=3,
        ),
    ...
    def _is_listening(self):
        ...
        try:
            ...
            if se.errno == errno.ENOPROTOOPT:
                raise RuntimeError(
                    "OS does not support SO_ACCEPTCONN, cannot determine socket state. Please supply an"
...
class SSL(Packet):
    ...
    name = "SSL/TLS"
    fields_desc = [
        PacketListField("records", None, None, next_cls_cb=lambda *args: self.guessed_next_layer)
    ]
    ...
    def do_dissect(self, raw_bytes):
        ...
        while pos < len(raw_bytes) - record_header_len:
            ...
            if self.tls_ctx is not None:
                payload = record(
                    raw_bytes[pos : pos + record_header_len + payload_len], ctx=self.tls_ctx
                )
    ...
    def do_decrypt_payload(self, record):
        ...
        if encrypted_payload is not None or (
            self.tls_ctx.negotiated.version is not None
            and self.tls_ctx.negotiated.version >= TLSVersion.TLS_1_3
        ):
            try:
                if self.tls_ctx.client:
                    cleartext = self.tls_ctx.server_ctx.crypto_ctx.decrypt(
                        encrypted_payload, record.content_type
                    )
                else:
                    cleartext = self.tls_ctx.client_ctx.crypto_ctx.decrypt(
                        encrypted_payload, record.content_type
                    )
                if self.tls_ctx.negotiated.version >= TLSVersion.TLS_1_3:
                    ...
                    try:
                        ...
                        raise TLSProtocolError(
                            "Decryption failed. Invalid 0x%02x content_type in payload"
                            % content_type,
                            response=record,
                        )
    ...
    def _get_encrypted_payload(self, record):
        ...
        if (
            record.haslayer(TLSRecord)
            and record[TLSRecord].content_type == TLSContentType.HANDSHAKE
...
cleartext_handler = {
    TLSPlaintext: lambda pkt, tls_ctx: (TLSContentType.APPLICATION_DATA, pkt[TLSPlaintext].data),
    TLSChangeCipherSpec: lambda pkt, tls_ctx: (
        TLSContentType.CHANGE_CIPHER_SPEC,
        pkt[TLSChangeCipherSpec].build(),
    ),
...
def to_raw(
    pkt, tls_ctx, include_record=True, compress_hook=None, pre_encrypt_hook=None, encrypt_hook=None
):
    from . import ssl_tls_crypto as tlsc
    ...
    if include_record:
        if tls_ctx.negotiated.version >= TLSVersion.TLS_1_3:
            ...
            tls_ciphertext = (
                TLSRecord(version=tls_ctx.negotiated.version, content_type=content_type)
                / ciphertext
            )
    else:
        ...
        if recv:
            ...
            if resp.haslayer(TLSAlert):
                ...
                if alert.level != TLSAlertLevel.WARNING:
                    ...
                    description = TLS_ALERT_DESCRIPTIONS.get(
                        alert.description, "unknown description"
                    )
                    raise TLSProtocolError(
                        "%s alert returned by server: %s" % (level.upper(), description.upper()),
                        pkt,
                        resp,
                    )
...
def tls_do_handshake(tls_socket, version, ciphers, extensions=[]):
    if version <= TLSVersion.TLS_1_2:
        client_hello = TLSRecord(version=version) / TLSHandshakes(
            handshakes=[
                TLSHandshake()
        ...
        client_key_exchange = TLSRecord(version=version) / TLSHandshakes(
            handshakes=[TLSHandshake() / tls_socket.tls_ctx.get_client_kex_data()]
        )
        ...
        resp2 = tls_do_round_trip(
            tls_socket,
            TLSHandshakes(
                handshakes=[TLSHandshake() / TLSFinished(data=tls_socket.tls_ctx.get_verify_data())]
            ),
        )
        return resp1, resp2
    else:
...
def tls_fragment_payload(pkt, record=None, size=2**14):
    ...
    if record is None:
        ...
        for payload in payloads:
            fragments.append(
                TLSRecord(
                    content_type=record.content_type, version=record.version, length=len(payload)
                )
                / payload
            )
            try:
...
bind_layers(
    TLSExtension, TLSExtALPN, {"type": TLSExtensionType.APPLICATION_LAYER_PROTOCOL_NEGOTIATION}
)
...
bind_layers(
    TLSExtension, TLSExtSignatureAlgorithms, {"type": TLSExtensionType.SIGNATURE_ALGORITHMS}
)
...
bind_layers(
    TLSExtension, TLSExtPSKKeyExchangeModes, {"type": TLSExtensionType.PSK_KEY_EXCHANGE_MODES}
)
