...
class TlsRecordGenerator:
    ...
    values = {
        ...
        'length': [0, (-17, -1, 0, 10, 0, 29, 1000)], # deltas to actual length
        # Handshake values.
        'hs_type': [0, (0, 1, 2, 3, 0xB, 0xC, 0xD, 0xE, 0xF, 0x10, 0x14, 0x1F)],
        'hs_length': [0, (-7, -2, 0, 11, 0, 51, 97)], # deltas to actual length
        # Payload lengths.
    ...
    def record(self):
        ...
        p_len = p_real_len + self.__curr_value('length') + 4
        if p_len < 0:
            p_len = 0
        hs_len = p_real_len + self.__curr_value('hs_length')
        if hs_len < 0:
            hs_len = 0
        msg = pack("!BBBHBBH",
                   self.__curr_value('type'),
                   self.__curr_value('major'), self.__curr_value('minor'),
                   p_len,
                   ...
                   hs_len)
        if p_real_len:
            msg += pack(str(p_real_len) + "s",
                        str(random.getrandbits(8 * p_real_len)))
        return msg
