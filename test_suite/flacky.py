...
class Filter(object):
    ...
    def __init__(self, node, direction=None):
        ...
        self.chain = "".join(["TempestaTestChain", self.node.type, self.direction])
    ...
    def drop_on_ports(self, dest_ports):
        ...
        for port in dest_ports:
            drop_cmd = "iptables -A %s -p tcp --dport %d -j DROP" % (self.chain, port)
    ...
    def clean_up(self):
        ...
        self.clean()
        remove_links_cmd = "iptables -D %s -j %s" % (self.direction, self.chain)
