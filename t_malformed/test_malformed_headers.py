...
class MalformedRequestsTestCase(tester.TempestaTest):
    ...
    tempesta = {
        'config': """
                listen ${tempesta_ip}:8414;
                srv_group default {
                    server ${server_ip}:8415;
                }
                vhost tempesta-cat {
                    proxy_pass default;
                }
    ...
    clients = [
        {
            ...
            'binary': 'curl',
            'ssl': False,
            'cmd_args': '-Ikf -v http://${tempesta_ip}:8414/ -H "Host: tempesta-tech.com:8414"',
        },
    ]
    ...
    def common_check(
        self,
        extra_header: str,
        expected_rc: int = http.HTTPStatus.OK,
    ):
        """
        Process common test checking.

        Args:
            extra_header (str): header to test
            expected_rc (int): expected http return code
        ...
        curl_cmd = curl.options[0]
        curl.options[0] = '{base_cmd} -H "{extra_header}"'.format(
            base_cmd=curl_cmd,
            extra_header=extra_header,
        )
        # TODO
        print('---CMD:', curl.options[0])

        curl.start()
        self.wait_while_busy(curl)
        response = curl.resq.get(True, 1)
        # TODO
        print('************', response)
        ...
        self.assertEqual(
            response.status,
            expected_rc,
            'Expected http code: {0}'.format(expected_rc),
        )
    ...
    def test_missing_header_name(self):
        ...
        """
        self.common_check(
            extra_header=': invalid header',
            expected_rc=http.HTTPStatus.BAD_REQUEST,
        )

    def test_accept_header_invalid(self):
        """
        Test header `Accept` with invalid data.

        https://tools.ietf.org/html/rfc7231#section-5.3.2
        """
        self.common_check(
            extra_header='Accept: invalid',
            expected_rc=http.HTTPStatus.BAD_REQUEST,
        )

    def test_accept_header_success(self):
        """
        Test header `Accept` with correct data.
    ...
    # TODO return 200
    def test_accept_charset_invalid(self):
        """
        Test header `Accept-Charset` with invalid data.
        ...
        """
        self.common_check(
            extra_header='Accept-Charset: invalid',
            expected_rc=http.HTTPStatus.BAD_REQUEST,
        )

    def test_accept_charset_success(self):
        """
        Test header `Accept-Charset` with correct data.
        ...
        """
        self.common_check(
            extra_header='Accept-Charset: utf-8',
        )

    # TODO return 200
    def test_accept_encoding_invalid(self):
        """
        Test header `Accept-Encoding`  with invalid data.

        https://tools.ietf.org/html/rfc7231#section-3.1.2.1
        """
        self.common_check(
            extra_header='Accept-Encoding: invalid',
            expected_rc=http.HTTPStatus.BAD_REQUEST,
        )

    def test_accept_encoding_success(self):
        """
        Test header `Accept-Encoding`  with invalid data.
