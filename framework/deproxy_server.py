...
class ServerConnection(asyncore.dispatcher):
    ...
    def handle_read(self):
        ...
        if self._server.drop_conn_when_receiving_data:
            self.close()
...
def deproxy_srv_factory(server: dict, name, tester):
    ...
    srv = StaticDeproxyServer(
        ...
        drop_conn_when_receiving_data=server.get("drop_conn_when_receiving_data", False),
        sleep_when_receiving_data=server.get("drop_conn_when_receiving_data", 0.0),
