...
class RatioDynamic(tester.TempestaTest):
    ...
    def calc_weight(self, servers, perfstat):
        ...
        cl_reqs = perfstat['client_requests'] = tempesta.stats.cl_msg_forwarded \
            - perfstat['tot_client_requests']
        perfstat['tot_client_requests'] = tempesta.stats.cl_msg_forwarded

        c_reqs = perfstat['const_server_requests'] = srv_const.requests - \
            perfstat['tot_const_server_requests']
        ...
        tf_cfg.dbg(3, "Total reqs %d; const_srv: reqs %d weight %d; "
                   "dyn_srv reqs %d weight %d "
                   % (cl_reqs, c_reqs, w_const, d_reqs, w_dyn))
    ...
    def test_load_distribution(self):
        ...
        self.assertGreater(w_const, w_dyn,
                           msg=("Slower server got higher weight. "
                                "%d vs %d"
                                % (w_const, w_dyn))
                          )
