...
def build_deproxy_unreliable(server, name, tester):
    ...
    if rtype == "static":
        ...
        srv = UnreliableDeproxyServer(port=port, response=content)
    else:
...
class TestCacheUseStale(TestCacheUseStaleBase):
    ...
    @parameterize.expand(
        [
            param(name="500", status=500, use_stale="500", r2_hdrs=[]),
            param(name="400", status=400, use_stale="400", r2_hdrs=[]),
            param(name="5xx_500", status=500, use_stale="5*", r2_hdrs=[]),
            param(name="5xx_502", status=502, use_stale="5*", r2_hdrs=[]),
            param(name="4xx_400", status=400, use_stale="4*", r2_hdrs=[]),
            param(name="4xx_403", status=403, use_stale="4*", r2_hdrs=[]),
            param(name="4xx&5xx", status=404, use_stale="4* 5*", r2_hdrs=[]),
            param(name="invalid_response", status=403, use_stale="5*", r2_hdrs=[("hd>r", "v")]),
        ]
    )
    ...
    @parameterize.expand(
        [
            param(name="s_maxage", status="400", r1_hdrs=[("cache-control", "s-maxage=1")]),
            param(
