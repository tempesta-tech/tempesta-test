...
def make_response(st_code, expected=True):
    resp_headers = ["Content-Length: 0", "Connection: keep-alive"]
    if expected:
        resp_headers += [
    ...
    response = deproxy.Response.create(
        status=st_code, headers=resp_headers, date=deproxy.HttpMessage.date_time_string()
    )
    return response
...
def response_500():
    ...
    headers = [
        "Date: %s" % date,
        "Content-Length: 0",
        "Connection: keep-alive" "Server: Tempesta FW/%s" % tempesta.version(),
    ]
...
def response_403(date=None, connection=None):
    if date is None:
        date = deproxy.HttpMessage.date_time_string()
    headers = ["Content-Length: 0", "Server: Tempesta FW/%s" % tempesta.version()]
    if connection != None:
        headers.append("Connection: %s" % connection)

    return deproxy.Response.create(status=403, headers=headers, date=date, body="")
...
def response_400(date=None, connection=None):
    ...
    resp = deproxy.Response.create(status=400, headers=headers, date=date, body="")
    return resp
...
def base(uri="/", method="GET", forward=True, date=None):
    ...
    if method == "PURGE":
        ...
        common_resp_headers += sample_headers + ["Last-Modified: Mon, 12 Dec 2016 13:59:39 GMT"]
        if method == "GET":
    ...
    base_chain = deproxy.MessageChain(
        request=client_req,
