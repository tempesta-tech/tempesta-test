...
"""
    ...
    def _check_servers_requests(self, server_listeners: list[str]) -> None:
        ...
        if "cache 0" not in self.cache_config:  # The current tests use the same uri
            for i, grp in enumerate(server_listeners):
                for listener in grp:
    ...
    def test(self, name, proto, tfw_port, http_option):
        with networker.change_mtu_and_restore_interfaces(mtu=self.mtu, disable_pmtu=False):
            with networker.create_and_cleanup_interfaces(
                    node=remote.server,
                    base_interface_name=tf_cfg.cfg.get("Server", "aliases_interface"),
                    base_interface_ip=tf_cfg.cfg.get("Server", "aliases_base_ip"),
                    number_of_ip=self.server_interfaces_n,
            ) as ips:
                ...
                self.get_tempesta().get_stats()


class TlsWrkStressDocker(tester.TempestaTest, check_memleak=True):
    """
