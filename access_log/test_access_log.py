...
class CheckedResponses(tester.TempestaTest):
    ...
    def make_request(self, uri, method="GET", version="1.1", request_body=None, **kwargs):
        request = [
            "{method} {uri} HTTP/{version}".format(method=method, uri=uri, version=version),
    ...
    def get_expected_log_msg(self):
        return (
            '"{method} {uri} HTTP/{version}" {status}'
    ...
    def check_response(self, klog, status, body):
        ...
        request = self.make_request(
            "/expect-status-%d" % status, referer=referer, user_agent=user_agent
        )
...
class AccessLogTest(CheckedResponses):
    ...
    def test_uri_truncate(self):
        ...
        self.assertEqual(
            msg.uri[: len("/too-long-uri_1111")], "/too-long-uri_1111", "Wrong uri prefix"
        )
